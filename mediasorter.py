...
parser.add_argument("--startingpath", "-sp", type=str, dest= "startpath", required=True)
parser.add_argument("--destinationpath", "-dp", type=str, dest= "destinationpath", required=True)
parser.add_argument("--photo", "-p", type=bool, dest= "photo", action=argparse.BooleanOptionalAction)
parser.add_argument("--video", "-v", type=bool, dest= "video", action=argparse.BooleanOptionalAction)
...
def main():
    now = datetime.now()
    videoext = (".mov", ".mp4", ".avi", ".m4v")
    photoext = (".gif", ".jpeg", ".jpg", ".png")
    ...
    if not args.photo and not args.video:
        logging.error("Neither Video nor Photo argument selected.")
        sys.exit(1)
    ...
    for file in filelist:
        ...
        os.chdir(args.startpath)
        if file.lower().endswith(videoext) and args.video is True:
        ...
        if file.lower().endswith(photoext) and args.photo is True:
            #Ensure that the "Begining Backup log is only given once and not given if nothing was done"
            if backupstart is None:
                logging.info("Begining Backup")
                backupstart = True
...
def metadataextract(file):
    ...
    try:
        creationdateraw = probe["format"]["tags"]["creation_time"]
        creationdate = creationdateraw.split("T")
        creationdate = creationdate[0]
        datesplit = creationdate.split("-")
        year = datesplit[0]
        month = datesplit[1]
